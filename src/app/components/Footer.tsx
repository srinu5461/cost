...
const { footer } = data;
const topCategories = (data.categoryTree || [])
  .filter((cat: any) => !cat.path || cat.path.split('/').filter(Boolean).length === 1)
  .slice(0, 12);
    return (
    <footer className="bg-[#0B1220] text-slate-300 w-full font-sans border-t border-slate-800/80">
      ...
      {/* ── SECTION 1: MAIN 4-COLUMN FOOTER GRID ── */}
    ...
    {topCategories.length > 0 ? topCategories.map((cat: any) => (
      ...
      <li>
        <Link to="/products" className="text-slate-300 hover:text-white transition-colors flex items-center gap-1.5 group">
          ...
          {/* Columns 2 & 3: Quick Links & Help & Support (Side-by-Side 2 Columns on Mobile) */}
          <div className="grid grid-cols-2 gap-4 sm:gap-8 md:col-span-2 md:grid-cols-2">

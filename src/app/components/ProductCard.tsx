...
const promisedDateInFuture = uropaPromisedDate ? new Date(uropaPromisedDate) > new Date() : false;
      ...
      return (
        <div className="flex flex-col mb-1 justify-end">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="bg-[#8B5CF6] text-white font-extrabold text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0.5 rounded-md uppercase tracking-wider shadow-xs">
              COST +$100
            </span>
          </div>
          <div className="flex items-baseline gap-1.5 mb-1.5">
  ...
  productBrandLogo ? (
    ...
    <span className="absolute top-2 left-2 text-xs font-black text-[#0284C7] uppercase tracking-wider z-10 max-w-[60%] truncate">
      {productBrand}
    </span>
  )
)}
  ...
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl p-4 flex flex-col justify-between hover:shadow-lg hover:border-slate-300 transition-all relative group h-full">
      <div>
        ...
        >
          {/* Out of Stock Badge directly upon image at top-left */}
          {!productInStock ? (
  ...
  productBrandLogo ? (
    ...
    <span className="absolute top-2 left-2 text-xs font-black text-[#0284C7] uppercase tracking-wider z-10 max-w-[50%] truncate">
      {productBrand}
    </span>
  )
) : null}
     ...
     {!uropaPromisedDate && uropaAvailabilityMessage && (
      <span className="text-[10px] text-amber-600 font-medium text-right">
        ...
        <Link to={`/products/${product.id}`} className="hover:text-[#E31837] transition-colors block mb-2 cursor-pointer">
          <h3 className="text-xs sm:text-sm font-extrabold text-[#0F172A] line-clamp-2 leading-snug h-[34px] sm:h-10">
            {productName}
          </h3>
        </Link>
